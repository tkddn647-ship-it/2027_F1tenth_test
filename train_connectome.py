...
def main():
    ...
    parser.add_argument("--resume", type=str, default=None,
                        help="이어서 학습할 PPO zip (예: connectome_ppo_lidar_ajou.zip)")
    ...
    if resume_path is None and Path(save_path).exists() and args.env == "lidar":
        # 같은 이름 zip이 있으면 기본으로 이어서
    ...
    logger = LapTimeLogger()
    model.learn(total_timesteps=args.timesteps, callback=logger, reset_num_timesteps=False)
